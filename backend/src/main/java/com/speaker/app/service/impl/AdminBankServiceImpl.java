...
@Service
public class AdminBankServiceImpl implements AdminBankServiceIntf {

    private final QuestionBankItemMapper bankMapper;

    public AdminBankServiceImpl(QuestionBankItemMapper bankMapper) {
    ...
    @Transactional
    @CacheEvict(cacheNames = { BankCacheNames.BANK_SEASONS, BankCacheNames.BANK_TOPIC_GROUPS }, allEntries = true)
    public void updateItem(long id, AdminBankItemUpdateRequest req) {
        ...
        QuestionBankItem row = QuestionBankItem.builder()
                ...
                .sortOrder(sortOrder)
                .build();
        int n = bankMapper.updateFieldsById(row);
        if (n == 0) {
