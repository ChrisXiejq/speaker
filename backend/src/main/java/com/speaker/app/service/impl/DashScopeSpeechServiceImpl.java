...
import com.speaker.app.service.intf.DashScopeSpeechServiceIntf;
 ...
 */
@Service
public class DashScopeSpeechServiceImpl implements DashScopeSpeechServiceIntf {
    ...
    public DashScopeSpeechServiceImpl(
    ...
    public String transcribeEnglish(byte[] audioBytes, String filenameHint) {
        ...
        String raw = withTransientRetry("ASR 请求", () -> dashScopeCompatibleClient.post()
    ...
    public byte[] synthesizeEnglish(String text) {
        ...
        String raw = withTransientRetry("TTS 请求", () -> dashScopeApiClient.post()
        ...
        try {
            ...
            if (url != null && !url.isBlank()) {
                byte[] bytes = withTransientRetry("TTS 音频下载", () -> RestClient.create()
                        .get()
                        .uri(URI.create(url))
                        .retrieve()
                        .body(byte[].class));
                if (bytes == null || bytes.length == 0) {
                    throw new IllegalStateException("TTS 音频下载为空");
                }
                return bytes;
            }
    ...
    private static <T> T withTransientRetry(String operation, Supplier<T> action) {
        ...
        for (int attempt = 1; attempt <= TRANSIENT_REQUEST_ATTEMPTS; attempt++) {
            try {
                ...
                try {
                    ...
                    throw new IllegalStateException(operation + "被中断", interrupted);
                }
            }
        }
        throw lastFailure != null ? lastFailure : new IllegalStateException(operation + "失败");
    }
}
